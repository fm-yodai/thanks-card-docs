...
| 状態管理 | Pinia + TanStack Query | 暫定 | UI状態は Pinia、サーバー状態は TanStack Query で役割を明確に分離 |
| バックエンド | Hono on AWS Lambda | 暫定 | TSファースト・Web標準APIベース・サーバーレスで運用負荷小 |
| API スタイル | Hono RPC（hono/client） | 暫定 | REST 互換で外販可・型安全・追加ライブラリ不要 |
| データベース | DynamoDB | チーム確認 | 月 $1 以下・フルマネージド・サーバーレス親和 |
| DB クライアント | ElectroDB | チーム確認 | Single Table Design 支援・型推論・SST 推奨 |
| 認証 | Cognito + Google OAuth | チーム確認 | AWS統一・IAM一元管理・5万ユーザー無料 |
...
│   │       ├── handlers/             ← SQS / EventBridge Lambda ハンドラー
...
### 状態管理方針（Pinia + TanStack Query）

設計原則:「UIの状態はPinia、サーバーの状態はTanStack Query」と役割を明確に分離する。フォルダ構造でも `stores/`（Pinia）と `queries/`（TanStack Query）を分けて視覚的に明示。

- **authStore（Pinia）** — Cognitoセッション・JWTトークン・ユーザー情報・isAdminフラグ
- **UI状態（Pinia）** — モーダルの開閉・フォーム入力中の一時データ・サイドバー展開状態
...
**採用:** TanStack Query の `refetchInterval` によるステータス条件付きポーリング。カードの `status === 'processing'` の間だけ 8 秒間隔でポーリング。`status` が `published` または `blocked` に変わった時点でポーリングが自動停止する。
...
export function useCardQuery(cardId: string) {
  return useQuery({
    ...
    // status が 'processing' の間だけ 8 秒ポーリング
    refetchInterval: (query) =>
      query.state.data?.status === 'processing' ? 8000 : false,
  })
}
```
...
| etl | EventBridge（月初 / オンデマンド） | 15 min | Python Lambda。DynamoDB → S3(Parquet) → QuickSight |
...
- **Conventional Commits** — `feat:` / `fix:` / `docs:` / `refactor:` 等でコミットメッセージを統一
...
- **E2E テスト** — MVP 後半で導入検討（Playwright が有力候補）
...
DynamoDB (OLTP) のデータを S3 (Parquet) にエクスポートし、分析する構成。管理者向けのWebダッシュボードは作らない。

| ツール | 月額 | ステータス |
|-------|------|----------|
| QuickSight | $5〜24/ユーザー | 検討中 |
| Grafana (OSS) | $0（セルフホスト） | 検討中 |
| Python + 静的レポート | $0 | 検討中 |

MVP では Python スクリプト + 静的レポート（S3 出力）が最もコスト効率が高い。

ETL方式は未定。DynamoDB Export to S3（ネイティブ機能）が60名規模のMVPでは最もシンプルで推奨。
...
- **AWS WAF** — API Gateway 前段に配置を検討中（~$5/月の追加コスト）
